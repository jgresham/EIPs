...
When a call is made to a contract (`FactoryContract`) which creates a new contract (`newContract`), the `FactoryContract` code is loaded into memory and should be warmed, but the `newContract` code is not loaded into memory currently, so it should not be warmed. The `newContract` account is warmed in clients currently.

`AContract newContract = FactoryContract.createAContract()`
